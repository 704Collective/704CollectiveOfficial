...
export default function PartnerTeamMessagesPage() {
  ...
  async function send() {
    ...
    if (!cid) {
      const ensured = await ensurePartnerTeamConversation();
      if (!ensured.ok) {
        setSending(false);
        toast.error(ensured.error);
        return;
      }
      cid = ensured.conversationId;
      setConversationId(cid);
    }
    const res = await postPartnerTeamThreadMessage(cid, text.trim());
    setSending(false);
    if (!res.ok) {
    ...
    await markPartnerTeamThreadRead(cid);
  }
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div className="rounded-xl border border-white/10 bg-white/[0.02] flex flex-col h-[min(70vh,560px)]">
        {!conversationId && (
          <p className="text-sm text-white/50 text-center px-4 pt-4">
            Your first message opens a thread with the 704 Collective team (all leadership are notified).
          </p>
        )}
        ...
        <div className="border-t border-white/10 p-4 flex flex-col sm:flex-row gap-2">
          <Textarea
            ...
            aria-label="Message the 704 Collective team"
